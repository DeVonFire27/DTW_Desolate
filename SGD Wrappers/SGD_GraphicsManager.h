...
namespace SGD
{
	...
	public:
		...
		virtual bool		DrawString			( const wchar_t* text, Point position,  Color color = {}, float rotation = 0.f)					= 0;
		virtual bool		DrawString			( const char* text, Point position,  Color color = {}, float = 0.f )							= 0;
		virtual bool		DrawLine			( Point position1, Point position2, Color color = {}, unsigned int lineWidth = 3 )				= 0;
		virtual bool		DrawRectangle		( Rectangle rect, Color fillColor, Color lineColor = {0,0,0,0}, unsigned int lineWidth = 3, float rotation = 0.f )	= 0;
		...
		void				SetTransform(D3DXMATRIX& value) { transform = value; }
