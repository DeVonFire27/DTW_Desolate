...
SGD::Vector SGD::Vector::Zero = SGD::Vector(0, 0);
namespace SGD
{
	//*****************************************************************//
...
#pragma region RECTANGLE_METHODS
	...
	bool Rectangle::IsClipping(const Rectangle& o) const
	{
		Rectangle intersection = ComputeIntersection(o);
		if (intersection == *this)
			return false;

		bool intersect_right = intersection.right == right;
		bool intersect_left = intersection.left == left;
		bool intersect_top = intersection.top == top;
		bool intersect_bottom = intersection.bottom == bottom;

		if ((intersect_top ^ intersect_bottom) && (intersect_right ^ intersect_left))
			return true;
...
#pragma region VECTOR_METHODS
	...
	Vector Vector::Trim(float maxDistance)
	{
		if (ComputeLength() < maxDistance)
			return *this;

		float angle = atan2(y, x);
		return Vector(cos(angle), sin(angle)) * maxDistance;
	}
	//*****************************************************************//
