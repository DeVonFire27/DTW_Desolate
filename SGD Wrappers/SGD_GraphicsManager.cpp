...
#include "globals.h"
// Comment this out to disable break on texture load failure
#define KILL_ON_TEX_FAILURE
// --------------------------------------------------------
...
namespace SGD
{
	namespace SGD_IMPLEMENTATION
	{
		...
		public:
			...
			virtual bool		DrawString				( const wchar_t* text, Point position,  Color color, float rotation )						override;
			virtual bool		DrawString				( const char* text, Point position,  Color color, float rotation)							override;
			virtual bool		DrawLine				( Point position1, Point position2, Color color, unsigned int width )						override;
			virtual bool		DrawRectangle			( Rectangle rect, Color fillColor, Color lineColor, unsigned int width, float rotation )	override;
	...
	namespace SGD_IMPLEMENTATION
	{
		...
		//*************************************************************//
		// DRAW STRING
		bool GraphicsManager::DrawString( const wchar_t* text, Point position, Color color, float rotation )
		{
			// Sanity-check the wrapper's status
			...
			D3DXMatrixRotationZ(&rot_matrix, rotation);

			// create world
			D3DXMATRIX world = rot_matrix * transform;
		...
		//*************************************************************//
		// DRAW STRING
		bool GraphicsManager::DrawString( const char* text, Point position, Color color, float rotation )
		{
			m_pSprite->SetTransform(&transform);
			...
			// Use the UTF16 draw
			return DrawString( m_pwszBuffer, position, color, rotation );
		}
		//*************************************************************//
		...
		//*************************************************************//
		// DRAW RECTANGLE
		bool GraphicsManager::DrawRectangle( Rectangle rect, Color fillColor, Color lineColor, unsigned int lineWidth, float rotation )
		{
			m_pSprite->SetTransform(&transform);
