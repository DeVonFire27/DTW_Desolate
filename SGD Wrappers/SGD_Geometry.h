...
#pragma region RECTANGLE_METHODS
		...
		bool		IsClipping			(const Rectangle& other) const; // returns true only if the edges of both rectangles intersect
...
#pragma region VECTOR_METHODS
		...
		// if this vectors length is longer than 'maxDistance' returns a vector with length 'maxDistance.' 
		// I.e. the vector is "trimed" to the length of 'maxDistance.'
		// Conversely, if 'maxDistance' is longer than the length of this vector, this same vector is returned
		Vector		Trim				(float maxDistance); 
